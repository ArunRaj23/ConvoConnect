...
navigator.mediaDevices.getUserMedia({
    ...
    document.getElementById('mark-attendance-btn').addEventListener('click', () => {
        // Emit an event to mark attendance
        console.log('Mark Attendance button clicked');
        socket.emit('get-attendance', ROOM_ID);
        console.log("event emitted")
    });


    const CLOUDINARY_URL = 'https://api.cloudinary.com/v1_1/dmccddall/upload';
const CLOUDINARY_UPLOAD_PRESET = 'GmeetProject';
...
document.getElementById('file-upload').addEventListener('change', async (event) => {
    ...
    document.getElementById('file-upload-btn').innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>';
...
console.log(data);
        if (data.secure_url) {
            ...
            // Send the file URL to the server via Socket.io
            socket.emit('file-uploaded', data.secure_url);

            // Reset the button
            document.getElementById('file-upload-btn').innerHTML = '<i class="fa-solid fa-upload"></i>';
...
socket.on('download-attendance', ({ filename, content }) => {
